      ...
      <main {...mainHandlers} className="relative w-full h-full flex flex-col z-10 bg-transparent">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* 1P: 메인 섹션 - [수정 1] 텍스트 잘림 해결을 위해 레이아웃 조정 */}
            {currentPage === 0 && (
              <div className="h-full flex flex-col px-8 text-center justify-between">
                <div className="pt-6">
                  ...
                  {/* 내부 여백(p-10 -> px-6 py-10)과 아이콘 간격(gap-6 -> gap-4)을 조절하여 공간 확보 */}
                  <div className="bg-white/95 backdrop-blur-md rounded-[3rem] px-6 py-10 flex flex-col gap-10 border border-purple-50 shadow-2xl shadow-purple-100/30 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 text-left">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <p className="text-slate-900 font-black text-[18px] leading-tight whitespace-nowrap overflow-visible">{meta.datetime}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-left">
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <p className="text-slate-900 font-black text-[17px] leading-tight whitespace-nowrap overflow-visible">{meta.venue}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {currentPage === 2 && (
              <div className="h-full flex flex-col">
                ...
                <div className="mx-8 mt-4 bg-purple-900 py-5 rounded-[2.5rem] text-center shadow-xl"><p className="text-white text-[13px] font-bold flex items-center justify-center gap-2"><Clock size={16} className="text-purple-300"/> 스크롤하여 전체 일정을 확인하세요</p></div>
              </div>
            )}

            {/* 4P: 오시는 길 - [수정 2] 배경의 느낌표 아이콘 제거 */}
            {currentPage === 3 && (
              <div className="h-full flex flex-col px-10 justify-between">
                <div>
                  ...
                  <div className="flex flex-col gap-6 text-center">
                    <div className="w-20 h-20 bg-purple-900 rounded-[2.5rem] flex items-center justify-center text-white mx-auto shadow-xl transform rotate-3"><MapPin size={40}/></div>
                    <div className="space-y-2"><p className="text-[22px] font-black text-slate-900 leading-snug break-keep">{location.address}</p><div className="h-1 w-10 bg-purple-100 mx-auto rounded-full" /></div>
                    <button onClick={() => window.open(location.naver_map_url, '_blank')} className="w-full bg-purple-600 text-white py-5 rounded-[2.5rem] font-black text-[18px] shadow-2xl shadow-purple-200 active:scale-95 transition-all">네이버 지도 앱 실행</button>
                  </div>
                </div>
                ...
                <div className="p-8 bg-purple-50/80 rounded-[3rem] border border-purple-100 space-y-4 text-left font-bold relative overflow-hidden shadow-sm">
                  <div className="flex items-center gap-4 relative z-10"><Phone size={18} className="text-purple-400"/><p className="text-slate-900 text-[16px]">{location.contact_name} : {location.contact_phone}</p></div>
                  {location.contact_email && (<div className="flex items-center gap-4 relative z-10"><Mail size={18} className="text-purple-400"/><p className="text-slate-900 text-[16px]">{location.contact_email}</p></div>)}
                </div>
              </div>
            )}

            {/* 5P: 기타 안내 - [수정 3] 스크롤바를 화면 우측 벽면에 밀착 */}
            {currentPage === 4 && (
              // 부모 컨테이너의 px-10을 제거하여 전체 너비 사용
              <div className="h-full flex flex-col">
                <div className="mb-8 px-10">
                  <h2 className="text-slate-900 text-[28px] font-black mb-2 border-b-4 border-purple-900 w-fit pb-1">기타 안내</h2>
                </div>
                
                {/* 스크롤 영역에 px-10을 적용하여 스크롤바를 바깥으로 밀어냄 */}
                <div className="flex-1 flex flex-col gap-10 overflow-y-auto items-center pt-2 pb-6 custom-scrollbar px-10">
                  
                  {/* 안내 이미지 (info.png) - 강제 로드 로직 포함 */}
