      ...
      <main {...mainHandlers} className="relative w-full h-full flex flex-col z-10 bg-transparent">
        ...
        <div className="absolute top-10 left-0 right-0 z-50 flex justify-center gap-3">
          {[...Array(totalPages)].map((_, i) => (
            <div 
              key={i} 
              className={`h-1 rounded-full transition-all duration-700 ease-in-out ${i === currentPage ? 'w-12 bg-purple-900 shadow-lg shadow-purple-200' : 'w-2.5 bg-purple-100'}`} 
            />
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* 4P: 오시는 길 - [수정] 문의 섹션 3줄 레이아웃 및 아이콘 크기 최적화 */}
            {currentPage === 3 && (
              <div className="h-full flex flex-col px-10 justify-between">
                ...
                {/* 하단 담당자 정보: 3줄 분리 및 사람 아이콘 추가 */}
                <div className="p-8 bg-purple-50/80 rounded-[3rem] border border-purple-100 space-y-5 text-left font-bold relative overflow-hidden shadow-sm mb-2">
                  <div className="flex items-center gap-4 relative z-10">
                    <User size={19} className="text-purple-400 shrink-0"/>
                    <p className="text-slate-900 text-[15px] leading-tight">{location.contact_name}</p>
                  </div>
                  <div className="flex items-center gap-4 relative z-10">
                    <Phone size={19} className="text-purple-400 shrink-0"/>
                    <p className="text-slate-900 text-[15px] leading-tight">{location.contact_phone}</p>
                  </div>
                  {location.contact_email && (
                    <div className="flex items-center gap-4 relative z-10">
                      <Mail size={19} className="text-purple-400 shrink-0"/>
                      <p className="text-slate-900 text-[15px] leading-tight break-all">{location.contact_email}</p>
                    </div>
                  )}
                </div>
              </div>
